...
class Tester():
    ...
    def eval_one_batch(self, data, net, class_dict):
        ...
        elem, _, _, element_atn, bg_score,_ = net(features)
        ...
        t = torch.range(0,bg_score.size(1)-1)
        # plt.plot(t,bg_score[0].squeeze().cpu(),t,element_atn[0].squeeze().cpu())
        # plt.legend(['bg','fg'])
    ...
    def final(self, logger=None, class_dict=None):
        ...
        # json_path = os.path.join('./ckpt/thumos', 'result_mean.json')
        # with open(json_path, 'w') as f:
        #     json.dump(self.final_res, f)
        #     f.close()
